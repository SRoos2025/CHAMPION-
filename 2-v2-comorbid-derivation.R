...
#go back to 1 row per ID
age <- age %>%
  group_by(id) %>%
  slice_head(n = 1)

#add age to comorbids
...
#some people do not have any comorbidity data and they are currently NA, we set that to 2 because at least we know they have kidney failure
combined_mortality_event <- combined_mortality_event %>%
  group_by(id) %>%
  mutate(
    cci = if_else(is.na(cci), 2, cci)
  ) %>%
  fill(cci, .direction = "downup") #fill it for every row
