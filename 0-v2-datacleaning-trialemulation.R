...
#we want to select their death date and their last event date
#note that total hosp days is not calculated here, but once we know their censor time
#because we just note the first hosp day and reason, we can later still filter when we know the censor time, so we can do this now.
...
#sometimes the hosp day was on day 0, but cox analysis works only when time until event is not 0. Of course, it is the first day so we change this to 1
event_select <- event_select %>%
  group_by(id) %>%
  mutate(
    cardiovasc_hosp_day = if_else(cardiovasc_hosp_day == 0, 1, cardiovasc_hosp_day),
    cardiovasc_hosp_day = if_else(is.na(cardiovasc_hosp_day), 0, cardiovasc_hosp_day),#then we can set NA to 0 as well, because we do not want it imputed
    infect_hosp_incl_covid_day = if_else(infect_hosp_incl_covid_day == 0, 1, infect_hosp_incl_covid_day),
    infect_hosp_incl_covid_day = if_else(is.na(infect_hosp_incl_covid_day), 0, infect_hosp_incl_covid_day),
    infect_hosp_excl_covid_day = if_else(infect_hosp_excl_covid_day == 0, 1, infect_hosp_excl_covid_day),
    infect_hosp_excl_covid_day = if_else(is.na(infect_hosp_excl_covid_day), 0, infect_hosp_excl_covid_day),
    cardiac_hosp_day = if_else(cardiac_hosp_day == 0, 1, cardiac_hosp_day),
...
combined_mortality_event <- combined_mortality_event %>%
  group_by(id) %>%
  mutate(
    cardiovasc_hosp_day = if_else(is.na(cardiovasc_hosp_day), 0, cardiovasc_hosp_day),#then we can set NA to 0 
    infect_hosp_incl_covid_day = if_else(is.na(infect_hosp_incl_covid_day), 0, infect_hosp_incl_covid_day),
    infect_hosp_excl_covid_day = if_else(is.na(infect_hosp_excl_covid_day), 0, infect_hosp_excl_covid_day),
    cardiac_hosp_day = if_else(is.na(cardiac_hosp_day), 0, cardiac_hosp_day),
    first_hosp = if_else(is.na(first_hosp), 0, first_hosp),
