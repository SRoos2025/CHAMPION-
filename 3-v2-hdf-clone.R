...
cohort_hdf <- cohort_hdf %>%
  ...
  mutate(
    ...
    death_reason_cardiovasc = if_else(is.infinite(death_reason_cardiovasc), 0, death_reason_cardiovasc),
    ...
    death_reason_infect_incl_covid = if_else(is.infinite(death_reason_infect_incl_covid), 0, death_reason_infect_incl_covid),
...
grace_cohort <- grace_cohort %>%
  ...
  mutate(
    days_from_fdd = case_when(
      two_week_period == 7 & days_from_fdd >90~ 90, # and the last should be max 90
      .default = days_from_fdd
    )) %>%
  ungroup()
...
cens_time_hdf <- cohort_hdf_reduced %>%
  group_by(id) %>%
  select(cens_time, id) %>%
  slice_head(n =1) %>%
  ungroup()

save(cens_time_hdf, file = paste0(path, "cens_time_hdf.Rdata"))
